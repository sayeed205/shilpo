...
pub use shilpo_device_protocol::DeviceCommand;
use shilpo_device_protocol::{self};
...
pub enum WorkerUpdate {
    ...
    CommandOutcome(shilpo_device_protocol::CommandOutcome),
}
...
fn emit_client_updates(
    updates: &UpdateSender,
    client: &DeviceClient,
    revisions: &mut std::collections::HashMap<shilpo_device_protocol::DeviceDomain, u64>,
) {
    use shilpo_device_protocol::DeviceDomain;
    for domain in [
        ...
        let update = match state.payload {
            ...
            shilpo_device_protocol::DomainPayload::Network(payload) => {
                Some(WorkerUpdate::Network(network_info(payload)))
            }
            shilpo_device_protocol::DomainPayload::Media(payload) => {
                Some(WorkerUpdate::Media(media_info(payload)))
            }
            shilpo_device_protocol::DomainPayload::Brightness(payload) => {
...
fn audio_info(payload: shilpo_device_protocol::AudioPayload) -> AudioInfo {
    fn device(v: shilpo_device_protocol::AudioDevicePayload) -> shilpo_services::AudioDevice {
...
fn media_info(p: shilpo_device_protocol::MediaPayload) -> MediaInfo {
...
fn brightness_info(p: shilpo_device_protocol::BrightnessPayload) -> BrightnessInfo {
    BrightnessInfo {
...
fn network_info(p: shilpo_device_protocol::NetworkPayload) -> NetworkInfo {
