...
impl SessionContext {
    ...
    pub fn init_with_paths(config_path: PathBuf, session_path: PathBuf, heed_dir: PathBuf) -> Self {
        let active_config =
            ShellConfig::load_or_create(&config_path).unwrap_or_else(|_| ShellConfig::default());
        ...
        let heed_store = match HeedSessionStore::open_with_recovery(&heed_dir) {
            Ok(opened) => {
                if let RecoveryOutcome::Quarantined { ref path } = opened.recovery {
                    tracing::warn!(
...
mod tests {
    ...
    fn test_session_context_init_with_paths() {
        let temp_base =
            std::env::temp_dir().join(format!("shilpo_session_test_{}", uuid::Uuid::new_v4()));
        let config_path = temp_base.join("config.toml");
        let session_path = temp_base.join("session.toml");
        let heed_dir = temp_base.join("heed");

        let context =
            SessionContext::init_with_paths(config_path.clone(), session_path.clone(), heed_dir);
