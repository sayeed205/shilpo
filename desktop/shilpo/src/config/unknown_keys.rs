...
fn shape_of_value(schema: &serde_json::Value, defs: &BTreeMap<String, serde_json::Value>) -> SchemaShape {
    if let Some(reference) = schema.get("$ref").and_then(serde_json::Value::as_str) {
        ...
        // Unresolvable reference: be permissive rather than inventing keys.
...
fn is_null_schema(schema: &serde_json::Value) -> bool {
    match schema.get("type") {
        ...
        Some(serde_json::Value::Array(many)) => {
            many.iter().all(|t| t.as_str() == Some("null"))
        }
        _ => false,
    }
}
...
fn walk_table(
    ...
    for key in keys {
        ...
        if let Some(field_shape) = fields.get(&key) {
            ...
            let (line, column) = locate(table, &key, key_span, value_span, text);
    ...
    for key in keys {
        ...
        if let Some(field_shape) = fields.get(&key) {
            if let Some(value) = table.get_mut(&key) {
                let mut item = Item::Value(value.clone());
                walk_item(&mut item, &subpath, field_shape, source, text, warnings);
            }
        } else if let Some(inner) = additional {
            if let Some(value) = table.get_mut(&key) {
                let mut item = Item::Value(value.clone());
                walk_item(&mut item, &subpath, inner, source, text, warnings);
            }
        } else {
            let (line, column) = locate(table, &key, key_span, value_span, text);
            let suggestion = best_suggestion(&key, fields.keys().map(String::as_str))
...
fn locate(
    ...
    locate_with_fallback(key_span, value_span, key, text)
}
...
mod tests {
    ...
    fn levenshtein_basic_distances() {
        ...
        assert_eq!(levenshtein_distance("widgets", "exclusive_zone"), 11);
    }
    ...
    fn suggestion_only_within_threshold() {
        ...
        assert_eq!(best_suggestion("widgts", ["widgets"]), None);
    }
    ...
    fn unknown_output_child_reports_typed_map_path() {
        ...
        assert_eq!(warning.key, "heigth");
        assert_eq!(warning.suggestion.as_deref(), Some("outputs.\"DP-1\".height"));
    }
    ...
    fn unknown_inline_table_key_is_removed_in_memory() {
        ...
        assert_eq!(warnings[0].suggestion.as_deref(), Some("bar.margin.vertical"));
