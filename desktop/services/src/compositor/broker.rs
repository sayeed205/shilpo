...
impl CommandTicket {
    ...
    /// Detaches the ticket, allowing the command to execute to completion in the background
    /// without being cancelled when the ticket is dropped.
...
impl ConvergenceState {
    fn evaluate(&mut self, snapshot: &CompositorSnapshot,) {
        ...
        let applied = match expectation {
            ...
            CommandExpectation::FocusPreviousWindow {
                previous_window_id,
            } => {
...
impl CompositorCommandBroker {
    ...
    pub fn submit_with_policy(
        ...
        let cap_ok = match &command {
            ...
            CompositorCommand::FocusWorkspace(_,) => {
                state.snapshot.capabilities.can_focus_workspace
            }
        ...
        loop {
            ...
            let outcome = if item.cancellation.is_cancelled() {
                ...
                match execution {
                    ...
                    Err(RejectionReason::Cancelled(reason,),) => {
                    ...
                    Ok(ack,) => match active.acknowledge(ack,) {
                        ...
                        Ok((),) => loop {
                            ...
                            let remaining =
                                item.deadline.saturating_duration_since(Instant::now(),);
...
pub fn create_stream_cancel_handle(stream: UnixStream,) -> Arc<dyn StreamCancelHandle,> {
    Arc::new(BasicStreamCancelHandle(Arc::new(
...
mod tests {
    ...
    fn test_reconnect_cancels_pending() {
        ...
        let t1 = broker
            ...
            .unwrap();
        let t2 = broker
            .submit(CompositorCommand::FocusWorkspace(2,),)
            .unwrap();
