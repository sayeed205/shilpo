...
use shilpo_services::compositor::{
    ...
    GenericWaylandCompositorBackend, HyprlandCompositorBackend, NiriCompositorService,
...
// ---------------------------------------------------------------------------
// HyprlandCompositorBackend Conformance Contract Tests
// ---------------------------------------------------------------------------

fn make_hyprland_service(
    capacity: usize,
    clock: &ManualClock,
    active_workspace: Arc<Mutex<Option<u64>>>,
    timeout_gate_rx_slot: Arc<Mutex<Option<std_mpsc::Receiver<()>>>>,
) -> Arc<HyprlandCompositorBackend> {
    let initial_snapshot = CompositorSnapshot {
        workspaces: default_workspaces(),
        ..Default::default()
    };
    ...
    let time_source: Arc<dyn TimeSource> = Arc::new(ManualTimeSource(clock.clone()));
    HyprlandCompositorBackend::new_offline_with(initial_snapshot, time_source, broker)
}

pub struct HyprlandCompositorDomainPortDriver {
    capacity: usize,
    service: Mutex<Arc<HyprlandCompositorBackend>>,
    clock: ManualClock,
    pending: Arc<Mutex<VecDeque<DriverPendingItem>>>,
    driver_supersessions: AtomicU64,
    active_workspace: Arc<Mutex<Option<u64>>>,
...
impl HyprlandCompositorDomainPortDriver {
    pub fn new(capacity: usize) -> Self {
        let active_workspace = Arc::new(Mutex::new(None));
        let timeout_gate_rx_slot = Arc::new(Mutex::new(None));
        let clock = ManualClock::new();
        let service = make_hyprland_service(
            capacity,
            &clock,
            active_workspace.clone(),
            timeout_gate_rx_slot.clone(),
    ...
    fn current_service(&self) -> Arc<HyprlandCompositorBackend> {
        self.service.lock().unwrap().clone()
    }
...
impl DomainPortDriver for HyprlandCompositorDomainPortDriver {
    type Payload = CompositorPayload;
    type Command = CompositorDriverCommand;
...
// ---------------------------------------------------------------------------
// Hyprland Scenario Test Suites
// ---------------------------------------------------------------------------

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_01_initial_projection_is_deterministic_and_unavailable() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_01_initial_projection_is_deterministic_and_unavailable(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_02_initial_start_follows_unavailable_connecting_ready() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_02_initial_start_follows_unavailable_connecting_ready(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_03_reconnect_retains_safe_payload_and_records_last_error() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_03_reconnect_retains_safe_payload_and_records_last_error(
        &driver,
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_04_strictly_newer_revision_in_same_generation_is_accepted() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_04_strictly_newer_revision_in_same_generation_is_accepted(
        &driver,
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_05_stale_generation_is_rejected() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_05_stale_generation_is_rejected(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_06_stale_revision_is_rejected() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_06_stale_revision_is_rejected(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_07_conflicting_payload_at_same_version_is_rejected_and_diagnosed() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_07_conflicting_payload_at_same_version_is_rejected_and_diagnosed(
        &driver,
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_08_new_owner_generation_permits_revision_reset() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_08_new_owner_generation_permits_revision_reset(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_09_slow_subscriber_converges_to_latest_atomic_snapshot() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_09_slow_subscriber_converges_to_latest_atomic_snapshot(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_10_accepted_command_receives_exactly_one_terminal_outcome() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_10_accepted_command_receives_exactly_one_terminal_outcome(
        &driver,
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_11_backend_acknowledgement_alone_does_not_complete_convergence_command()
{
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_11_backend_acknowledgement_alone_does_not_complete_convergence_command(&driver);
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_12_lossless_mailbox_rejects_overflow_without_dropping_accepted_commands()
{
    let driver = HyprlandCompositorDomainPortDriver::new(2);
    domain_port_contract::scenario_12_lossless_mailbox_rejects_overflow_without_dropping_accepted_commands(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_13_replace_latest_supersedes_pending_command_with_same_key_and_emits_terminal_cancellation()
 {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_13_replace_latest_supersedes_pending_command_with_same_key_and_emits_terminal_cancellation(&driver);
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_14_different_replace_latest_keys_do_not_replace_each_other() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_14_different_replace_latest_keys_do_not_replace_each_other(
        &driver,
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_17_five_failures_inside_60_seconds_enter_quarantine() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_17_five_failures_inside_60_seconds_enter_quarantine(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_18_five_minutes_stable_clears_rolling_failure_window_but_preserves_session_restart_telemetry()
 {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_18_five_minutes_stable_clears_rolling_failure_window_but_preserves_session_restart_telemetry(&driver);
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_19_quarantine_requires_explicit_reset_or_containing_process_restart() {
    let driver = HyprlandCompositorDomainPortDriver::new(10);
    domain_port_contract::scenario_19_quarantine_requires_explicit_reset_or_containing_process_restart(&driver);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn hyprland_scenario_20_telemetry_reports_generation_queue_depth_capacity_overloads_supersessions_restarts_stale_updates_and_last_error()
 {
    let driver = HyprlandCompositorDomainPortDriver::new(2);
    domain_port_contract::scenario_20_telemetry_reports_generation_queue_depth_capacity_overloads_supersessions_restarts_stale_updates_and_last_error(&driver);
}
