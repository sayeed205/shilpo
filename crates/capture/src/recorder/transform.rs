//! Output transform helpers for converting screen-space regions into
//! captured-frame (buffer) space.
//!
//! Adapted from `wl-screenrec` (`src/transform.rs`), licensed under the
...
impl Rect {
    ...
    /// Convert a rectangle from screen space into the coordinate space of the
    /// captured frame (which is pre-transform buffer space).
...
/// Whether the transform transposes the axes (90 and 270 degree rotations and
/// their flipped variants).
pub fn transform_is_transposed(transform: Transform) -> bool {
    transform_basis(transform).0[0] == 0
}

/// Swap a dimension pair when the output transform transposes the axes.
...
mod test {
    ...
    use super::{Rect, transform_is_transposed};
