//! Thin FFmpeg `AVAudioFifo` wrapper for resampling the fixed-size Opus
//! encoder input.
//!
//! Adapted from `wl-screenrec` (`src/fifo.rs`), licensed under the Apache
...
impl Drop for AudioFifo {
    fn drop(&mut self) {
        unsafe { av_audio_fifo_free(self.0.as_ptr()) }
    }
}
