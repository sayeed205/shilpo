...
`shilpo-shell` exposes a secure UNIX domain socket at `$XDG_RUNTIME_DIR/shilpo/shell.sock` (mode `0700`).
...
## 6. Systemd User Service Example

Place the following service unit in `~/.config/systemd/user/shilpo-shell.service`:

```ini
[Unit]
Description=Shilpo Desktop Shell
Documentation=https://github.com/sayeed/shilpo
After=graphical-session.target
Requisite=graphical-session.target

[Service]
ExecStart=/usr/local/bin/shilpo-shell
Restart=on-failure
RestartSec=3s
...
- **Socket Errors**: Verify `$XDG_RUNTIME_DIR` ownership and permissions (`chmod 0700 $XDG_RUNTIME_DIR/shilpo`).
