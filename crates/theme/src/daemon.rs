...
use crate::state::{SideEffect, ThemeCommand, ThemeMode, ThemeState, reduce};
use anyhow::Result;
    ...
    async fn process_command(&mut self, command: ThemeCommand) -> Result<ThemeState, String> {
        if let ThemeCommand::SetColorSource(source) = &command {
            let available = match source {
                crate::state::ColorSource::Custom => self.state.custom_seed.is_some(),
                crate::state::ColorSource::Wallpaper => self.state.wallpaper_seed.is_some(),
            };
            if !available {
